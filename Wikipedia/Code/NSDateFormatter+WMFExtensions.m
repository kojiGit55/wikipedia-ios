...
+ (instancetype)wmf_englishUTCNonDelimitedYearMonthDayHourMinuteSecondFormatter {
    static NSDateFormatter *_dateFormatter;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _dateFormatter = [self wmf_newEnglishYearMonthDayHourMinuteSecondFormatter];
        _dateFormatter.timeZone = [NSTimeZone timeZoneWithAbbreviation:@"UTC"];
    });
    return _dateFormatter;
}
...
+ (instancetype)wmf_newEnglishYearMonthDayHourMinuteSecondFormatter {
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
